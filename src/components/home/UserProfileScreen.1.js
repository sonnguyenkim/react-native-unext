...
class UserProfileScreen extends Component {
  ...
  isServiceProviderHandler = (value) => {
    if (value) {
      Alert.alert(
        ...
        [
          {text: 'NO', onPress: () => {}, style: 'cancel'},
          {text: 'YES', onPress: () => this.setState({userInfo: {...this.state.userInfo, isServiceProvider: value}, isChanged: true, isInfoChanged: true} )}
        ],
        { cancelable: false, onDismiss: () => {} }
      );
    }
    else {
      this.setState({userInfo: {...this.state.userInfo, isServiceProvider: value}, isChanged: true, isInfoChanged: true} );
    }
  }
  ...
  componentDidUpdate(prevProps) {
    const { error, logged, loading, uploading, uploaderror, uploadsuccess, authUpdateProfileSuccess } = this.props;
    ...
    if (this.state.isInfoChanged && this.state.isAvatarChanged) {
      if (uploadsuccess) {
        ...
        this.setState({avatarSource: null, isAvatarChanged: false});
      }
      if (authUpdateProfileSuccess) {
        this.props.authUpdateProfileDone();
        this.setState({ isChanged: false, isInfoChanged: false});
      }
    } else if (this.state.isInfoChanged) {
      // console.log('this.state.isInfoChanged-only')
      if (authUpdateProfileSuccess) {
        // console.log('this.state.isInfoChanged-only-updateprofilesuccess')
        this.props.authUpdateProfileDone();
        this.setState({isChanged: false, isInfoChanged: false});
      }
    } else if (this.state.isAvatarChanged) {
